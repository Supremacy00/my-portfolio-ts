...
import { worksData, Work } from "../../constant/data";
...
const AllWorks = () => {
  ...
  const allWorks: Work[] = [...allWorksSet]
    .map((title) => {
      ...
      const symphony = worksData.symphonies.find(
        (work) => work.title === title,
      );
        ...
        <article className="grid grid-cols-1 gap-5 pb-12 lg:grid-cols-2 px-7 pt-7">
          {sortedWorksAphabetically.map((work) => (
            <motion.article
              ...
              className="group w-full border border-gray-200 dark:border-custom2 rounded-2xl p-5 overflow-hidden"
            >
              <div>
                <Link
                  href={work.Link}
